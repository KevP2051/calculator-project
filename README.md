...
A calculator application built for the Sezzle technical assessment. It pairs a React + TypeScript interface with a small Go REST microservice, supporting addition, subtraction, multiplication, division, exponentiation, square root, and percentage calculations.
...
Browser
  │
  ▼
React + TypeScript frontend ──HTTP/JSON──► Go REST microservice
http://localhost:5173                       http://localhost:8080
```

The frontend sends each calculation to `POST /api/v1/calculate`. The microservice validates the request, performs the calculation, and returns a JSON result or a predictable JSON error.

```text
calculator-project/
├── frontend/                 React, TypeScript, Vite, Tailwind, and tests
├── backend/                  Go microservice, arithmetic domain, and tests
...
Open [http://localhost:5173](http://localhost:5173). The frontend reads `VITE_API_URL` from `frontend/.env`; its default points to the local microservice.

## Run with Docker

Docker runs the frontend as a production static site and the Go microservice as its own container. No Node.js or Go installation is needed on the host.
...
Unit tests live next to the code they verify:

| Layer | Test location | Focus |
|---|---|---|
| Backend | `backend/internal/api/`, `backend/internal/service/`, and `backend/internal/calc/` (`*_test.go`) | HTTP behavior, validation, arithmetic operations, and numerical guards |
| Frontend | `frontend/src/calculator/**/*.test.ts(x)` | Form validation, calculator interactions, error states, and display formatting |

Run tests from their respective directories. Both commands print a coverage summary in the terminal.
...
Open [backend/coverage.html](backend/coverage.html) in a browser to inspect the per-line report. From the `backend` directory, use the command for your operating system:
...
The report is written to [frontend/coverage/index.html](frontend/coverage/index.html). From the `frontend` directory, open it with:
...
- **Stable error experience:** the API returns a stable error code, such as `DIVISION_BY_ZERO`; the frontend maps that code to its own user-facing message. This keeps API diagnostics separate from UI copy and prevents the client from relying on the microservice's wording.
...
- **Presentation-level number formatting:** the microservice preserves the value it calculated, while `formatResult` rounds only for display to 15 significant digits and uses scientific notation for extreme values. This hides familiar floating-point noise without changing the API contract.
- **Behavior-focused tests:** frontend tests mock the Axios instance and the toast library, nothing else. The form, schema, hook, rendering, and error handling run together, so tests follow the user-visible path rather than asserting implementation details such as CSS classes.

### Microservice rationale

- **Spec-driven development:** the microservice's requirements, clarification work, research, plan, and task breakdown were created with GitHub Spec Kit before implementation. This process is documented in the [microservice design decisions](backend/README.md#why-spec-driven-development) and the [`specs/001-calculator-rest-api/`](specs/001-calculator-rest-api/) directory.
- **Topology sized to the problem:** one stateless Go microservice serves the entire server-side contract, and the React client calls it directly over HTTP. A gateway, BFF, broker, or additional services would introduce operational and coordination costs without solving a problem present in this assessment.
...
- **Multi-stage images:** the microservice image compiles a small Go binary, while the frontend image builds the Vite application and serves static assets through Nginx. This keeps runtime images free of the respective build toolchains.
- **Nginx rather than the Vite server:** `npm run dev` and `npm run preview` are development tools, not production servers. A Vite build produces plain HTML, CSS and JavaScript, so the container does not need to run the application, only to serve files; Nginx does that and leaves Node.js and `node_modules` out of the shipped image entirely.
- **Nginx serves files only:** it does not reverse-proxy `/api` to the microservice. The browser calls the API directly, the same way it does when running locally, so the deployment does not introduce a request path that local development never exercises.
...
- The system uses one microservice. Given the application's size and the absence of cross-service concerns, the React client calls that service directly over HTTP. An API gateway, backend-for-frontend layer, message broker, and additional services would add complexity without a corresponding benefit for this scope.
...
- [Microservice guide](backend/README.md#calculator-microservice): setup, operations, API contract, and design rationale.
- [Frontend coverage report](frontend/coverage/index.html): interactive HTML report generated by Vitest.
- [Microservice coverage report](backend/coverage.html): per-line HTML report generated by Go's coverage tools.
